    ...
    let rec add_field_accesses prefix_path acc = function
      ...
      | access :: access_list ->
          ...
          then add_field_accesses prefix_path' acc access_list
            ...
            let snapshot_opt =
              AccessSnapshot.make_access formals prefix_path' ~is_write loc locks acqs critical_pair threads pre
            in
...
(*       let () = print_endline "\n =========================================\n HIPPODROME (make_container_access): " in
      ...
      let callee_access =
        AccessSnapshot.make_container_access formals receiver_ap ~is_write callee_pname callee_loc
          astate.locks astate.lock_state critical_pair astate.threads ownership_pre
      in
      let ownership =
  ...
  let add_callee_accesses formals (caller_astate : Domain.t) callee_accesses locks threads actuals
    ...
    let update_callee_access (snapshot : AccessSnapshot.t) acc =
      ...
      in
      let critical_pair = (CriticalPairs.choose_opt caller_astate.critical_pairs) in
      let snapshot_opt =
        AccessSnapshot.update_callee_access formals snapshot callsite ownership_precondition threads
          locks caller_astate.lock_state critical_pair
      in
      (* let () = print_string "\n snapshot_opt: \n" in
       * let () = match snapshot_opt with
       *   |Some snapshot_opt -> Domain.AccessSnapshot.pp Format.std_formatter snapshot_opt  
    ...
    let astate_callee =
      ...
      if is_thread_utils_method "assertMainThread" callee_pname then
        ...
        | NoEffect -> (
            ...
            | Some {threads; locks; critical_pairs; accesses; return_ownership; return_attribute} ->
                let locks =
...
let describe_exp = MF.wrap_monospaced RacerDFixDomain.pp_exp

let describe_pname = MF.wrap_monospaced (Procname.pp_simplified_string ~withclass:true)

let pp_access fmt (t : RacerDFixDomain.AccessSnapshot.t) =
  ...
  | InterfaceCall _ as access ->
      RacerDFixDomain.Access.pp fmt access
...
module ReportMap : sig
  ...
  module Key = struct
    ...
    let of_access (access : RacerDFixDomain.Access.t) =
      match access with
      | Read {exp} | Write {exp} ->
...
let should_report_on_proc tenv procdesc =
  ...
  | Java java_pname ->
      ...
      RacerDFixModels.is_thread_safe_method proc_name tenv
...
let should_report_guardedby_violation classname ({snapshot; tenv; procname} : reported_access) =
  ...
  && RacerDFixDomain.AccessSnapshot.is_write snapshot
  && Procname.is_java procname
  &&
  (* restrict check to access paths of length one *)
  match
    RacerDFixDomain.Access.get_access_exp snapshot.elem.access
  ...
  let report_unsafe_access accesses acc
    ...
    | (Read _ | ContainerRead _) when Procname.is_java pname ->
        ...
        let can_conflict (snapshot1 : AccessSnapshot.t) (snapshot2 : AccessSnapshot.t) =
          ...
          && not (RacerDFixDomain.different_locks snapshot1.elem.locks snapshot2.elem.locks) then false
